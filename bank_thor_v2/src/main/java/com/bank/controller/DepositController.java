...
public class DepositController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session == null)
		{
			...
			long accountNumber=transaction.getAccountNumber();		
			 try {
				 ...
				 Transaction transaction1=new Transaction(accountNumber, amount, openingBalance,openingBalance+amount , "Deposit");
					...
					session.setAttribute("accountNumbert", transaction1.getAccountNumber());
					session.setAttribute("closingBalance", transaction1.getClosingBalance());
			...
			// response.sendRedirect("successt");
